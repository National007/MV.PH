...
public class FileManager : IHttpHandler
{
	public void ProcessRequest(HttpContext context)
	{
		...
		//根目录路径，相对路径
		String rootPath = "/resource/editor/";
		//根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/resource/
		String rootUrl ="/resource/editor/";
		//图片扩展名
		...
		{
			context.Response.Write("不允许使用..移动到上一级目录");
			context.Response.End();
		}
		//最后一个字符不是/
		if (path != "" && !path.EndsWith("/"))
		{
			context.Response.Write("最后一个字符不是/");
			context.Response.End();
		}
		//目录不存在或不是目录
